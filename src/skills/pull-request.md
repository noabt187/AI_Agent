...
2. PR 提交是带副作用的发布操作，不走需求分析确认；确认 PR 参数时必须输出 `action=confirm` 且 `confirmType="design"`，禁止使用 `confirmType="requirement"`。
...
8. 调用 createPullRequest 前，必须使用 `action=confirm, confirmType="design"` 向用户确认 PR 参数：rootDir、repoUrl、prRepoUrl、baseBranch、headBranch、headOwner/auto、title、body、commitMessage、draft。不要在用户未确认前调用 createPullRequest。
