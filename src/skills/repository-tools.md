...
这些工具都是带副作用的仓库操作。调用前必须输出 `action=confirm` 且 `confirmType="design"` 让用户确认参数；不要用 `confirmType="requirement"`。
...
3. 调用前用 `action=confirm, confirmType="design"` 让用户看清楚源仓库、目标账号/组织、fork 名称、是否只 fork 默认分支。
...
5. 调用前用 `action=confirm, confirmType="design"` 确认本地目标目录、remote 名称和可选 upstream。
