...
这些工具都有副作用。Fork 使用 `request_confirmation(scope="remote_git")`；Clone 使用 `request_confirmation(scope="workspace_write")`。确认前不要尝试调用对应工具。
...
3. 调用前用 `request_confirmation(scope="remote_git")` 让用户看清楚源仓库、目标账号/组织、fork 名称、是否只 fork 默认分支。
...
5. 调用前用 `request_confirmation(scope="workspace_write")` 确认本地目标目录、remote 名称和可选 upstream。
