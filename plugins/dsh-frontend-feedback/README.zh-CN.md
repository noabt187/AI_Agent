...
文件编辑器保存时保留 UTF-8 BOM、纯 LF/CRLF/CR 换行，以及文件末尾是否存在换行。混合换行文件只读展示并明确提示，不会自动统一换行。输入后撤销回磁盘内容时，文件恢复为“已保存”状态。

丢弃草稿、载入磁盘版本会在浏览器缓存清理成功后同步编辑区并清空旧撤销记录。普通输入和保存保留编辑器视图、选区及撤销历史。保存期间的新修改仍未保存，并以新磁盘版本为基准缓存；丢弃期间的新修改会保留。缓存失败与磁盘保存成功分别提示。旧草稿键和原始内容继续兼容：旧记录没有格式信息且基准 hash 一致时沿用磁盘格式，否则进入冲突处理。
...
5. **文字修改事务层**：PageCraft PPT 沿 `slideId → deck.json → slide.content → 元素路径` 精确定位；普通网页继续使用受限的静态源码分析。两条路径都只写入经过验证的本地目标，刷新后检查真实 DOM，失败时按文件哈希安全回滚。演示清单只负责 PPT 数据语义和受管理图片绑定。
