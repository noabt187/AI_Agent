...
function extractTextDocument(extension, bytes) {
  try {
    return {
      extension,
...
async function extractDocumentByExtension(extension, bytes) {
  switch (extension) {
    case ".pdf":
...
async function extractPresentationDocument(fileName, bytes, maxTextCharacters = DEFAULT_MAX_EXTRACTED_TEXT_CHARACTERS) {
...
function presentationJobDirectory(cwd, jobId) {
...
async function createPresentationSource(cwd, fileName, bytes, options = {}) {
...
async function readRequestBodyWithLimit(req, maxBytes = DEFAULT_MAX_DOCUMENT_BYTES) {
...
async function handlePresentationSource(req, res, ctx, config) {
  ...
  try {
    ...
    const body = await readRequestBodyWithLimit(req, config.maxDocumentBytes ?? DEFAULT_MAX_DOCUMENT_BYTES);
