...
function buildAnnotationPrompt(comments, options = {}) {
  ...
  return [
    ...
    "\u5C06 request \u4F5C\u4E3A\u5B8C\u6574\u529F\u80FD\u9700\u6C42\u5904\u7406\uFF0C\u5148\u8FFD\u8E2A\u76F8\u5173\u7EC4\u4EF6\u3001\u72B6\u6001\u3001API\u3001\u670D\u52A1\u7AEF\u903B\u8F91\u548C\u6D4B\u8BD5\uFF0C\u5224\u65AD\u9700\u8981\u4FEE\u6539\u54EA\u4E9B\u5C42\u3002",
    "\u53EA\u6D89\u53CA\u89C6\u89C9\u65F6\u53EF\u53EA\u6539\u524D\u7AEF\uFF1B\u6D89\u53CA\u4E1A\u52A1\u80FD\u529B\u65F6\u5B8C\u6210\u5B9E\u9645\u9700\u8981\u7684\u524D\u7AEF\u4EA4\u4E92\u3001\u72B6\u6001\u7BA1\u7406\u3001\u63A5\u53E3\u8C03\u7528\u3001\u540E\u7AEF\u63A5\u53E3/\u6570\u636E\u903B\u8F91\u3001\u9519\u8BEF\u5904\u7406\u548C\u6D4B\u8BD5\uFF0C\u5F62\u6210\u5B8C\u6574\u4FEE\u6539\u529F\u80FD\u95ED\u73AF\uFF0C\u4E0D\u6269\u5C55\u5230\u65E0\u5173\u6A21\u5757\u3002",
    "\u9075\u5B88\u5BBF\u4E3B\u7684\u786E\u8BA4\u548C\u6743\u9650\u6D41\u7A0B\uFF1B\u9700\u8981\u6388\u6743\u65F6\u5148\u8BF4\u660E\u65B9\u6848\u4E0E\u64CD\u4F5C\u8303\u56F4\uFF0C\u7B49\u5F85\u7528\u6237\u786E\u8BA4\u540E\u518D\u6267\u884C\u3002",
    "",
...
import { parseFragment as parseFragment3 } from "parse5";
...
function htmlAttribute(node, name2) {
  return node.attrs?.find((attribute2) => attribute2.name.toLowerCase() === name2.toLowerCase())?.value;
}
...
function inlineImageSources(slide) {
  if (typeof slide.content !== "string") return [];
  const fragment = parseFragment3(slide.content);
  return htmlElements(fragment).filter((node) => node.tagName?.toLowerCase() === "img").map((node) => htmlAttribute(node, "src")).filter((src) => typeof src === "string");
}
function escapeHtmlAttribute(value) {
  return value.replaceAll("&", "&amp;").replaceAll('"', "&quot;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
}
function imageStyle(value, fit, position) {
  const declarations = (value ?? "").split(";").map((declaration) => declaration.trim()).filter((declaration) => declaration.length > 0 && !/^object-(?:fit|position)\s*:/i.test(declaration));
  declarations.push(`object-fit: ${fit}`, `object-position: ${position}`);
  return declarations.join("; ");
}
function imageStartTag(node, src, alt, fit, position) {
  const attributes = (node.attrs ?? []).filter((attribute2) => !["src", "alt", "style"].includes(attribute2.name.toLowerCase()));
  attributes.push(
    { name: "src", value: src },
    { name: "alt", value: alt },
    { name: "style", value: imageStyle(htmlAttribute(node, "style"), fit, position) }
  );
  return `<img${attributes.map((attribute2) => ` ${attribute2.name}="${escapeHtmlAttribute(attribute2.value)}"`).join("")}>`;
}
function replaceInlineImage(content, slotId, src, alt, fit, position) {
  const fragment = parseFragment3(content, { sourceCodeLocationInfo: true });
...
async function bindPresentationProjectAsset(cwd, input, options = {}) {
  ...
  if (slotId !== void 0) {
    ...
    const matchingSlides = deckSlides(deck).flatMap((slide) => {
      if (input.slideId !== void 0 && slide.id !== input.slideId) return [];
      if (typeof slide.content !== "string") return [];
    ...
    const match = typeof input.imageKey === "string" ? /^([a-zA-Z0-9][a-zA-Z0-9_-]{0,79})\.visual$/.exec(input.imageKey) : null;
