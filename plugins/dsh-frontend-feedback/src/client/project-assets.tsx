...
const projectAssetStyles: Record<string, CSSProperties> = {
  overlay: { position: 'absolute', inset: 0, zIndex: 50, display: 'grid', placeItems: 'center', padding: 22, background: inkTheme.overlayStrong, backdropFilter: 'blur(5px)' },
  dialog: { width: 'min(980px, 94vw)', height: 'min(720px, 88vh)', display: 'grid', gridTemplateRows: 'auto auto minmax(0,1fr) auto', overflow: 'hidden', border: `1px solid ${inkTheme.lineStrong}`, borderTop: `3px solid ${inkTheme.accent}`, borderRadius: 10, color: inkTheme.text, background: inkTheme.canvas, boxShadow: inkTheme.shadow },
  header: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 18px', borderBottom: `1px solid ${inkTheme.line}`, background: inkTheme.surfaceRaised },
  title: { display: 'block', fontFamily: inkTypography.serif, fontSize: 18, fontWeight: 600, letterSpacing: '.04em' },
  subtitle: { display: 'block', marginTop: 3, color: inkTheme.muted, fontSize: 11 },
  closeButton: { width: 32, height: 32, border: `1px solid ${inkTheme.line}`, borderRadius: 5, color: inkTheme.textSoft, background: inkTheme.surface, cursor: 'pointer', fontSize: 20 },
  slotNotice: { padding: '9px 18px', borderBottom: `1px solid ${inkTheme.line}`, color: inkTheme.textSoft, background: inkTheme.accentSoft, fontSize: 12 },
  ...
  assetCard: { minWidth: 0, display: 'grid', gap: 5, padding: 8, textAlign: 'left', border: `1px solid ${inkTheme.line}`, borderRadius: 7, color: inkTheme.text, background: inkTheme.surface, cursor: 'pointer', boxShadow: '0 2px 8px rgba(48, 43, 33, .04)' },
  assetCardActive: { borderColor: inkTheme.lineStrong, boxShadow: `inset 0 -3px 0 ${inkTheme.cinnabar}`, background: inkTheme.accentSoft },
  thumbnail: { width: '100%', aspectRatio: '16/10', objectFit: 'cover', borderRadius: 5, background: inkTheme.surfaceMuted },
  assetName: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 11 },
  assetMeta: { color: inkTheme.muted, fontSize: 10 },
  reference: { color: inkTheme.warning, fontSize: 10 },
  inspector: { minHeight: 0, overflowY: 'auto', display: 'grid', alignContent: 'start', gap: 13, padding: 15, borderLeft: `1px solid ${inkTheme.line}`, background: inkTheme.surface },
  previewBox: { aspectRatio: '16/10', overflow: 'hidden', border: `1px solid ${inkTheme.line}`, borderRadius: 6, background: inkTheme.surfaceMuted },
  previewImage: { width: '100%', height: '100%', display: 'block' },
  field: { display: 'grid', gap: 6, color: inkTheme.textSoft, fontSize: 11 },
  select: { height: 34, padding: '0 8px', border: `1px solid ${inkTheme.line}`, borderRadius: 5, color: inkTheme.text, background: inkTheme.surfaceRaised },
  empty: { gridColumn: '1/-1', display: 'grid', placeItems: 'center', minHeight: 150, color: inkTheme.muted, fontSize: 12 },
  deleteButton: { height: 34, border: `1px solid ${inkTheme.cinnabarLine}`, borderRadius: 5, color: inkTheme.cinnabar, background: inkTheme.cinnabarSoft, cursor: 'pointer' },
  footer: { display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto auto', alignItems: 'center', gap: 10, padding: '12px 15px', borderTop: `1px solid ${inkTheme.line}`, background: inkTheme.surfaceRaised },
  status: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: inkTheme.muted, fontSize: 11 },
  secondaryButton: { height: 36, padding: '0 13px', border: `1px solid ${inkTheme.line}`, borderRadius: 5, color: inkTheme.textSoft, background: inkTheme.surface, cursor: 'pointer' },
  primaryButton: { height: 36, padding: '0 16px', border: 0, borderRadius: 5, color: '#fff', background: inkTheme.inkFill, cursor: 'pointer', fontWeight: 800 },
}
