...
const initial: SessionDetail = { id: 'A', title: 'Session A', running: false, messages: [], state: {
  ...
  task: { id: 'task-A', revision: 2, objective: 'A objective', phase: 'awaiting_confirmation', previousContext: [], completedTaskIds: [], failedTaskIds: [], pendingConfirmation: proposal, lastRunId:'previous' },
} , runs:[{ id:'previous', sessionId:'A', prompt:'A', status:'completed', createdAt:1, messageIds:[], taskId:'task-A', taskRevision:2 }] }
...
test('mounted App sends the displayed proposal identity for confirm, revise, cancel and resume', async t => {
  ...
  const dialogProto = (dom.window as any).HTMLDialogElement.prototype
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (url === '/api/sessions/B') return json({ ...initial, id: 'B', title: 'Session B', runs: [{ ...initial.runs![0], sessionId:'B', taskId:'task-B' }], state: { sessionId: 'B', allowedPaths: [], task: { ...initial.state.task, id: 'task-B', objective: 'B objective', phase: 'paused', pendingConfirmation: undefined } } })
  ...
  const click = async (text: string) => { const button = [...dom.document.querySelectorAll('button')].find(b => b.textContent?.trim() === text); assert.ok(button, `button ${text}`); await act(async () => button.click()) }
  const render = async (key: string) => { await act(async () => root.render(<App key={key} pluginRuntime={runtime} />)) }
  await render('confirm')
  assert.match(dom.document.body.textContent!, /完整方案正文（不能丢失）/)
  await click('确认执行')
  assert.deepEqual(requests.at(-1)?.body.control, { kind: 'confirm', ...ref })
  assert.match(dom.document.body.textContent!, /完整方案正文（不能丢失）/, 'rejection must retain proposal')
  assert.match(dom.document.body.textContent!, /方案或运行已变化/)
  await click('调整方案')
  const textarea = dom.document.querySelector<HTMLTextAreaElement>('#host-task-revision')!
  ...
  await act(async () => { dom.document.querySelector<HTMLInputElement>('input[value="选项乙：写入"]')!.click() })
  await click('确认执行')
  assert.deepEqual(requests.at(-1)?.body, { prompt: '确认', control: { kind: 'confirm', ...ref, selection: '选项乙：写入' }, origin:'composer' })
  detail = structuredClone(initial)
  ...
  assert.doesNotMatch(dom.document.body.textContent!, /确认后允许 Agent 按本方案修改/)
  await click('继续分析')
  assert.deepEqual(requests.at(-1)?.body.control, { kind: 'confirm', ...ref })
  ...
  assert.match(dom.document.body.textContent!, /已确认方案：选项乙：写入/)
  await click('继续任务')
